#ifndef PPO_HPP_INCLUDED
#define PPO_HPP_INCLUDED
...
enum class Const_PPO_series_type{
    none,
    close,
    open,
    high,
...
{
public:
    static const map<Const_PPO_interval, string> s_interval_translation;
    static const map<Const_PPO_series_type, string> s_series_type_translation;
    static const map<Const_PPO_matype, string> s_matype_translation;
};

class MetaData_Type_PPO
{
private:
    string Symbol;
    string Indicator;
...
public:
    MetaData_Type_PPO();
    MetaData_Type_PPO(const MetaData_Type_PPO& other); // copy constructor
...
class TechnicalIndicator_Type_PPO
{
private:
    string PPO;
    string DateTime;
...
public:
    TechnicalIndicator_Type_PPO();
    TechnicalIndicator_Type_PPO(const TechnicalIndicator_Type_PPO& other); // copy constructor

    string get_PPO() const {return PPO;};
    string get_DateTime() const {return DateTime;};
    void set_PPO(string PPO){this->PPO = PPO;};
    void set_DateTime(string DateTime){this->DateTime = DateTime;};

   int ParseInternal(string& str_data, jsmntok_t* array_token, int index, int token_size);
};

class AvapiResponse_PPO_Content
{
private:
    MetaData_Type_PPO MetaData;
    list<TechnicalIndicator_Type_PPO> TechnicalIndicator;
    string str_prefix;
    bool error_status;
    string ErrorMessage;

public:
    AvapiResponse_PPO_Content();
    ~AvapiResponse_PPO_Content() {};
    AvapiResponse_PPO_Content(const AvapiResponse_PPO_Content& other); // copy constructor
    MetaData_Type_PPO& get_MetaData() {return MetaData;};
    list<TechnicalIndicator_Type_PPO>& get_TechnicalIndicator() {return TechnicalIndicator;};
...
class AvapiResponse_PPO
{
private:
    string LastHttpRequest;
    string RawData;
    AvapiResponse_PPO_Content* Data;

public:
    AvapiResponse_PPO();
    AvapiResponse_PPO(const AvapiResponse_PPO& other); // copy constructor
    AvapiResponse_PPO(AvapiResponse_PPO&& other); // move constructor
    AvapiResponse_PPO& operator=(const AvapiResponse_PPO& other); //copy assignment
    AvapiResponse_PPO& operator=(AvapiResponse_PPO&& other); //move assignment
    ~AvapiResponse_PPO();

    AvapiResponse_PPO_Content& get_Data() const {return *Data;};
...
private:
    ...
    static Impl_PPO *s_instance;
    Impl_PPO() {};
    static const unsigned int START_TOKEN_SIZE = 3000;
    static void ParseInternal(AvapiResponse_PPO& Response);

public:
    static Impl_PPO& getInstance();
    ...
    AvapiResponse_PPO Query(string symbol
        ,Const_PPO_interval interval
        ,Const_PPO_series_type series_type
        ,string fastperiod
        ,string slowperiod
        ,Const_PPO_matype matype
        );
