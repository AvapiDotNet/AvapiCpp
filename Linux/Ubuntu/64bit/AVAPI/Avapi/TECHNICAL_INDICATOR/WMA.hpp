#ifndef WMA_HPP_INCLUDED
#define WMA_HPP_INCLUDED
...
enum class Const_WMA_series_type{
    none,
    close,
    open,
    high,
...
{
public:
    static const map<Const_WMA_interval, string> s_interval_translation;
    static const map<Const_WMA_series_type, string> s_series_type_translation;
};

class MetaData_Type_WMA
{
private:
    string Symbol;
    string Indicator;
...
public:
    MetaData_Type_WMA();
    MetaData_Type_WMA(const MetaData_Type_WMA& other); // copy constructor
...
class TechnicalIndicator_Type_WMA
{
private:
    string WMA;
    string DateTime;
...
public:
    TechnicalIndicator_Type_WMA();
    TechnicalIndicator_Type_WMA(const TechnicalIndicator_Type_WMA& other); // copy constructor

    string get_WMA() const {return WMA;};
    string get_DateTime() const {return DateTime;};
    void set_WMA(string WMA){this->WMA = WMA;};
    void set_DateTime(string DateTime){this->DateTime = DateTime;};

   int ParseInternal(string& str_data, jsmntok_t* array_token, int index, int token_size);
};

class AvapiResponse_WMA_Content
{
private:
    MetaData_Type_WMA MetaData;
    list<TechnicalIndicator_Type_WMA> TechnicalIndicator;
    string str_prefix;
    bool error_status;
    string ErrorMessage;

public:
    AvapiResponse_WMA_Content();
    ~AvapiResponse_WMA_Content() {};
    AvapiResponse_WMA_Content(const AvapiResponse_WMA_Content& other); // copy constructor
    MetaData_Type_WMA& get_MetaData() {return MetaData;};
    list<TechnicalIndicator_Type_WMA>& get_TechnicalIndicator() {return TechnicalIndicator;};
...
class AvapiResponse_WMA
{
private:
    string LastHttpRequest;
    string RawData;
    AvapiResponse_WMA_Content* Data;

public:
    AvapiResponse_WMA();
    AvapiResponse_WMA(const AvapiResponse_WMA& other); // copy constructor
    AvapiResponse_WMA(AvapiResponse_WMA&& other); // move constructor
    AvapiResponse_WMA& operator=(const AvapiResponse_WMA& other); //copy assignment
    AvapiResponse_WMA& operator=(AvapiResponse_WMA&& other); //move assignment
    ~AvapiResponse_WMA();

    AvapiResponse_WMA_Content& get_Data() const {return *Data;};
...
private:
    ...
    static Impl_WMA *s_instance;
    Impl_WMA() {};
    static const unsigned int START_TOKEN_SIZE = 3000;
    static void ParseInternal(AvapiResponse_WMA& Response);

public:
    static Impl_WMA& getInstance();
    ...
    AvapiResponse_WMA Query(string symbol
        ,Const_WMA_interval interval
        ,string time_period
        ,Const_WMA_series_type series_type
        );
