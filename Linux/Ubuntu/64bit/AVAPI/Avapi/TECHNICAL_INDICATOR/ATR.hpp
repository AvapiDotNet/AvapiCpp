#ifndef ATR_HPP_INCLUDED
#define ATR_HPP_INCLUDED
...
{
public:
    static const map<Const_ATR_interval, string> s_interval_translation;
};

class MetaData_Type_ATR
{
private:
    string Symbol;
    string Indicator;
...
public:
    MetaData_Type_ATR();
    MetaData_Type_ATR(const MetaData_Type_ATR& other); // copy constructor
...
class TechnicalIndicator_Type_ATR
{
private:
    string ATR;
    string DateTime;
...
public:
    TechnicalIndicator_Type_ATR();
    TechnicalIndicator_Type_ATR(const TechnicalIndicator_Type_ATR& other); // copy constructor

    string get_ATR() const {return ATR;};
    string get_DateTime() const {return DateTime;};
    void set_ATR(string ATR){this->ATR = ATR;};
    void set_DateTime(string DateTime){this->DateTime = DateTime;};

   int ParseInternal(string& str_data, jsmntok_t* array_token, int index, int token_size);
};

class AvapiResponse_ATR_Content
{
private:
    MetaData_Type_ATR MetaData;
    list<TechnicalIndicator_Type_ATR> TechnicalIndicator;
    string str_prefix;
    bool error_status;
    string ErrorMessage;

public:
    AvapiResponse_ATR_Content();
    ~AvapiResponse_ATR_Content() {};
    AvapiResponse_ATR_Content(const AvapiResponse_ATR_Content& other); // copy constructor
    MetaData_Type_ATR& get_MetaData() {return MetaData;};
    list<TechnicalIndicator_Type_ATR>& get_TechnicalIndicator() {return TechnicalIndicator;};
...
class AvapiResponse_ATR
{
private:
    string LastHttpRequest;
    string RawData;
    AvapiResponse_ATR_Content* Data;

public:
    AvapiResponse_ATR();
    AvapiResponse_ATR(const AvapiResponse_ATR& other); // copy constructor
    AvapiResponse_ATR(AvapiResponse_ATR&& other); // move constructor
    AvapiResponse_ATR& operator=(const AvapiResponse_ATR& other); //copy assignment
    AvapiResponse_ATR& operator=(AvapiResponse_ATR&& other); //move assignment
    ~AvapiResponse_ATR();

    AvapiResponse_ATR_Content& get_Data() const {return *Data;};
...
private:
    ...
    static Impl_ATR *s_instance;
    Impl_ATR() {};
    static const unsigned int START_TOKEN_SIZE = 3000;
    static void ParseInternal(AvapiResponse_ATR& Response);

public:
    static Impl_ATR& getInstance();
    ...
    AvapiResponse_ATR Query(string symbol
        ,Const_ATR_interval interval
        ,string time_period
        );
