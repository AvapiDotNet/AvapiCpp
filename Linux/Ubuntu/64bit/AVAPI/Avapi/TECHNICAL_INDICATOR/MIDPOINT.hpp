#ifndef MIDPOINT_HPP_INCLUDED
#define MIDPOINT_HPP_INCLUDED
...
enum class Const_MIDPOINT_interval{
    none,
    n_1min,
    n_5min,
    n_15min,
...
enum class Const_MIDPOINT_series_type{
    none,
    close,
    open,
    high,
...
{
public:
    static const map<Const_MIDPOINT_interval, string> s_interval_translation;
    static const map<Const_MIDPOINT_series_type, string> s_series_type_translation;
};

class MetaData_Type_MIDPOINT
{
private:
    string Symbol;
    string Indicator;
...
public:
    MetaData_Type_MIDPOINT();
    MetaData_Type_MIDPOINT(const MetaData_Type_MIDPOINT& other); // copy constructor
...
class TechnicalIndicator_Type_MIDPOINT
{
private:
    string MIDPOINT;
    string DateTime;
...
public:
    TechnicalIndicator_Type_MIDPOINT();
    TechnicalIndicator_Type_MIDPOINT(const TechnicalIndicator_Type_MIDPOINT& other); // copy constructor

    string get_MIDPOINT() const {return MIDPOINT;};
    string get_DateTime() const {return DateTime;};
    void set_MIDPOINT(string MIDPOINT){this->MIDPOINT = MIDPOINT;};
    void set_DateTime(string DateTime){this->DateTime = DateTime;};

   int ParseInternal(string& str_data, jsmntok_t* array_token, int index, int token_size);
};

class AvapiResponse_MIDPOINT_Content
{
private:
    MetaData_Type_MIDPOINT MetaData;
    list<TechnicalIndicator_Type_MIDPOINT> TechnicalIndicator;
    string str_prefix;
    bool error_status;
    string ErrorMessage;

public:
    AvapiResponse_MIDPOINT_Content();
    ~AvapiResponse_MIDPOINT_Content() {};
    AvapiResponse_MIDPOINT_Content(const AvapiResponse_MIDPOINT_Content& other); // copy constructor
    MetaData_Type_MIDPOINT& get_MetaData() {return MetaData;};
    list<TechnicalIndicator_Type_MIDPOINT>& get_TechnicalIndicator() {return TechnicalIndicator;};
...
class AvapiResponse_MIDPOINT
{
private:
    string LastHttpRequest;
    string RawData;
    AvapiResponse_MIDPOINT_Content* Data;

public:
    AvapiResponse_MIDPOINT();
    AvapiResponse_MIDPOINT(const AvapiResponse_MIDPOINT& other); // copy constructor
    AvapiResponse_MIDPOINT(AvapiResponse_MIDPOINT&& other); // move constructor
    AvapiResponse_MIDPOINT& operator=(const AvapiResponse_MIDPOINT& other); //copy assignment
    AvapiResponse_MIDPOINT& operator=(AvapiResponse_MIDPOINT&& other); //move assignment
    ~AvapiResponse_MIDPOINT();

    AvapiResponse_MIDPOINT_Content& get_Data() const {return *Data;};
...
private:
    ...
    static Impl_MIDPOINT *s_instance;
    Impl_MIDPOINT() {};
    static const unsigned int START_TOKEN_SIZE = 3000;
    static void ParseInternal(AvapiResponse_MIDPOINT& Response);

public:
    static Impl_MIDPOINT& getInstance();
    ...
    AvapiResponse_MIDPOINT Query(string symbol
        ,Const_MIDPOINT_interval interval
        ,string time_period
        ,Const_MIDPOINT_series_type series_type
        );
