#ifndef RSI_HPP_INCLUDED
#define RSI_HPP_INCLUDED
...
enum class Const_RSI_series_type{
    none,
    close,
    open,
    high,
...
{
public:
    static const map<Const_RSI_interval, string> s_interval_translation;
    static const map<Const_RSI_series_type, string> s_series_type_translation;
};

class MetaData_Type_RSI
{
private:
    string Symbol;
    string Indicator;
...
public:
    MetaData_Type_RSI();
    MetaData_Type_RSI(const MetaData_Type_RSI& other); // copy constructor
...
class TechnicalIndicator_Type_RSI
{
private:
    string RSI;
    string DateTime;
...
public:
    TechnicalIndicator_Type_RSI();
    TechnicalIndicator_Type_RSI(const TechnicalIndicator_Type_RSI& other); // copy constructor

    string get_RSI() const {return RSI;};
    string get_DateTime() const {return DateTime;};
    void set_RSI(string RSI){this->RSI = RSI;};
    void set_DateTime(string DateTime){this->DateTime = DateTime;};

   int ParseInternal(string& str_data, jsmntok_t* array_token, int index, int token_size);
};

class AvapiResponse_RSI_Content
{
private:
    MetaData_Type_RSI MetaData;
    list<TechnicalIndicator_Type_RSI> TechnicalIndicator;
    string str_prefix;
    bool error_status;
    string ErrorMessage;

public:
    AvapiResponse_RSI_Content();
    ~AvapiResponse_RSI_Content() {};
    AvapiResponse_RSI_Content(const AvapiResponse_RSI_Content& other); // copy constructor
    MetaData_Type_RSI& get_MetaData() {return MetaData;};
    list<TechnicalIndicator_Type_RSI>& get_TechnicalIndicator() {return TechnicalIndicator;};
...
class AvapiResponse_RSI
{
private:
    string LastHttpRequest;
    string RawData;
    AvapiResponse_RSI_Content* Data;

public:
    AvapiResponse_RSI();
    AvapiResponse_RSI(const AvapiResponse_RSI& other); // copy constructor
    AvapiResponse_RSI(AvapiResponse_RSI&& other); // move constructor
    AvapiResponse_RSI& operator=(const AvapiResponse_RSI& other); //copy assignment
    AvapiResponse_RSI& operator=(AvapiResponse_RSI&& other); //move assignment
    ~AvapiResponse_RSI();

    AvapiResponse_RSI_Content& get_Data() const {return *Data;};
...
private:
    ...
    static Impl_RSI *s_instance;
    Impl_RSI() {};
    static const unsigned int START_TOKEN_SIZE = 3000;
    static void ParseInternal(AvapiResponse_RSI& Response);

public:
    static Impl_RSI& getInstance();
    ...
    AvapiResponse_RSI Query(string symbol
        ,Const_RSI_interval interval
        ,string time_period
        ,Const_RSI_series_type series_type
        );
