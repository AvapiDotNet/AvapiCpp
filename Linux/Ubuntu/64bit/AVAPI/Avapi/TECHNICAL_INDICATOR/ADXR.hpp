#ifndef ADXR_HPP_INCLUDED
#define ADXR_HPP_INCLUDED
...
enum class Const_ADXR_interval{
    none,
    n_1min,
    n_5min,
    n_15min,
...
{
public:
    static const map<Const_ADXR_interval, string> s_interval_translation;
};

class MetaData_Type_ADXR
{
private:
    string Symbol;
    string Indicator;
...
public:
    MetaData_Type_ADXR();
    MetaData_Type_ADXR(const MetaData_Type_ADXR& other); // copy constructor
...
class TechnicalIndicator_Type_ADXR
{
private:
    string ADXR;
    string DateTime;
...
public:
    TechnicalIndicator_Type_ADXR();
    TechnicalIndicator_Type_ADXR(const TechnicalIndicator_Type_ADXR& other); // copy constructor

    string get_ADXR() const {return ADXR;};
    string get_DateTime() const {return DateTime;};
    void set_ADXR(string ADXR){this->ADXR = ADXR;};
    void set_DateTime(string DateTime){this->DateTime = DateTime;};

   int ParseInternal(string& str_data, jsmntok_t* array_token, int index, int token_size);
};

class AvapiResponse_ADXR_Content
{
private:
    MetaData_Type_ADXR MetaData;
    list<TechnicalIndicator_Type_ADXR> TechnicalIndicator;
    string str_prefix;
    bool error_status;
    string ErrorMessage;

public:
    AvapiResponse_ADXR_Content();
    ~AvapiResponse_ADXR_Content() {};
    AvapiResponse_ADXR_Content(const AvapiResponse_ADXR_Content& other); // copy constructor
    MetaData_Type_ADXR& get_MetaData() {return MetaData;};
    list<TechnicalIndicator_Type_ADXR>& get_TechnicalIndicator() {return TechnicalIndicator;};
...
class AvapiResponse_ADXR
{
private:
    string LastHttpRequest;
    string RawData;
    AvapiResponse_ADXR_Content* Data;

public:
    AvapiResponse_ADXR();
    AvapiResponse_ADXR(const AvapiResponse_ADXR& other); // copy constructor
    AvapiResponse_ADXR(AvapiResponse_ADXR&& other); // move constructor
    AvapiResponse_ADXR& operator=(const AvapiResponse_ADXR& other); //copy assignment
    AvapiResponse_ADXR& operator=(AvapiResponse_ADXR&& other); //move assignment
    ~AvapiResponse_ADXR();

    AvapiResponse_ADXR_Content& get_Data() const {return *Data;};
...
private:
    ...
    static Impl_ADXR *s_instance;
    Impl_ADXR() {};
    static const unsigned int START_TOKEN_SIZE = 3000;
    static void ParseInternal(AvapiResponse_ADXR& Response);

public:
    static Impl_ADXR& getInstance();
    ...
    AvapiResponse_ADXR Query(string symbol
        ,Const_ADXR_interval interval
        ,string time_period
        );
