...
#include "Avapi/TECHNICAL_INDICATOR/HT_DCPERIOD.hpp"
...
int main()
{
    ...
    auto& QueryObject = avapi_connection->GetQueryObject_HT_DCPERIOD();

    auto Response = QueryObject.Query(   
             "MSFT"
             ,Const_HT_DCPERIOD_interval::weekly
             ,Const_HT_DCPERIOD_series_type::close);

    cout << endl <<  "******** RAW DATA HT_DCPERIOD ********"<< endl;
    cout <<  Response.get_RawData() << endl << endl;

    cout << "******** STRUCTURED DATA HT_DCPERIOD ********"<< endl;
    ...
    else
    {
        ...
        {
            cout << "DCPERIOD: " << element.get_DCPERIOD() << endl;
