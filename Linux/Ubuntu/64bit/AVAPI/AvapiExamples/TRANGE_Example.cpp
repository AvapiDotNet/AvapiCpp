...
#include "Avapi/TECHNICAL_INDICATOR/TRANGE.hpp"
...
int main()
{
    ...
    auto& QueryObject = avapi_connection->GetQueryObject_TRANGE();

    auto Response = QueryObject.Query(   
             "MSFT"
             ,Const_TRANGE_interval::n_60min);

    cout << endl <<  "******** RAW DATA TRANGE ********"<< endl;
    cout <<  Response.get_RawData() << endl << endl;

    cout << "******** STRUCTURED DATA TRANGE ********"<< endl;
    ...
    else
    {
        ...
        {
            cout << "TRANGE: " << element.get_TRANGE() << endl;
