...
#include "Avapi/TIME_SERIES/TIME_SERIES_INTRADAY.hpp"
...
int main()
{
    ...
    auto& QueryObject = avapi_connection->GetQueryObject_TIME_SERIES_INTRADAY();

    auto Response = QueryObject.Query(   
             "MSFT"
             ,Const_TIME_SERIES_INTRADAY_interval::n_60min
             ,Const_TIME_SERIES_INTRADAY_outputsize::compact
             ,Const_TIME_SERIES_INTRADAY_datatype::json);

    cout << endl <<  "******** RAW DATA TIME SERIES INTRADAY ********"<< endl;
    cout <<  Response.get_RawData() << endl << endl;

    cout << "******** STRUCTURED DATA TIME SERIES INTRADAY ********"<< endl;
