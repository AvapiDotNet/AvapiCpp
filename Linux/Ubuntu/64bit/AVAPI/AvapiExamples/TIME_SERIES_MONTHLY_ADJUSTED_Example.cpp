...
#include "Avapi/TIME_SERIES/TIME_SERIES_MONTHLY_ADJUSTED.hpp"
...
int main()
{
    ...
    auto& QueryObject = avapi_connection->GetQueryObject_TIME_SERIES_MONTHLY_ADJUSTED();

    auto Response = QueryObject.Query(   
             "MSFT"
             ,Const_TIME_SERIES_MONTHLY_ADJUSTED_datatype::json);

    cout << endl <<  "******** RAW DATA TIME SERIES MONTHLY ADJUSTED ********"<< endl;
    cout <<  Response.get_RawData() << endl << endl;

    cout << "******** STRUCTURED DATA TIME SERIES MONTHLY ADJUSTED ********"<< endl;
