...
#include "Avapi/TECHNICAL_INDICATOR/HT_TRENDLINE.hpp"
...
int main()
{
    ...
    auto& QueryObject = avapi_connection->GetQueryObject_HT_TRENDLINE();

    auto Response = QueryObject.Query(   
             "MSFT"
             ,Const_HT_TRENDLINE_interval::n_60min
             ,Const_HT_TRENDLINE_series_type::close);

    cout << endl <<  "******** RAW DATA HT_TRENDLINE ********"<< endl;
    cout <<  Response.get_RawData() << endl << endl;

    cout << "******** STRUCTURED DATA HT_TRENDLINE ********"<< endl;
    ...
    else
    {
        ...
        {
            cout << "HT_TRENDLINE: " << element.get_HT_TRENDLINE() << endl;
