...
int main()
{
    ...
    auto Response = QueryObject.Query(   
             "MSFT"
             ,Const_TIME_SERIES_DAILY_ADJUSTED_outputsize::compact
             ,Const_TIME_SERIES_DAILY_ADJUSTED_datatype::json);

    cout << endl <<  "******** RAW DATA TIME SERIES DAILY ADJUSTED ********"<< endl;
    cout <<  Response.get_RawData() << endl << endl;

    cout << "******** STRUCTURED DATA TIME SERIES DAILY ADJUSTED ********"<< endl;
    ...
    else
    {
        ...
        for(auto& element : TimeSeries)
        {
            ...
            cout << "SplitCoefficient: " << element.get_SplitCoefficient() << endl;
