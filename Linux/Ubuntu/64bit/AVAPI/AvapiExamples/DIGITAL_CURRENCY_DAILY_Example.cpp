...
#include "Avapi/DIGITAL_CURRENCY/DIGITAL_CURRENCY_DAILY.hpp"
...
int main()
{
    ...
    auto& QueryObject = avapi_connection->GetQueryObject_DIGITAL_CURRENCY_DAILY();

    auto Response = QueryObject.Query(   
             "BTC"
             ,"CNY"
             ,Const_DIGITAL_CURRENCY_DAILY_datatype::json);

    cout << endl <<  "******** RAW DATA DIGITAL CURRENCY DAILY ********"<< endl;
    cout <<  Response.get_RawData() << endl << endl;

    cout << "******** STRUCTURED DATA DIGITAL CURRENCY DAILY ********"<< endl;
